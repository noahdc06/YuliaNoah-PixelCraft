...
public class Jigsaw extends BaseConverter {
    ...
    private void shuffleTiles(int tileX, int tileY) {
        ...
        if (tileX >= width) {
        	// End of column reached/exceeded: calls itself, moving to the next row of tiles
            ...
            //Calls itself, iterating through each tile column
    ...
    private void swapPixels(int x1, int y1, int x2, int y2, int dx, int dy) {
        ...
        if (dx >= 50) {
            ...
            int px2 = x2 + dx;
            int py2 = y2 + dy;
            //Checks to ensure the pixel is within bounds, otherwise call next immediately
